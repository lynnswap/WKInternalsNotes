# ``WKInternalsNotes/_WKTextManipulationItem``

## Topics

### Type

#### Properties
- ``WKInternalsNotes/_WKTextManipulationItem/debugDescription``
- ``WKInternalsNotes/_WKTextManipulationItem/identifier``
- ``WKInternalsNotes/_WKTextManipulationItem/isCrossSiteSubframe``
- ``WKInternalsNotes/_WKTextManipulationItem/isSubframe``
- ``WKInternalsNotes/_WKTextManipulationItem/tokens``

#### Methods
- ``WKInternalsNotes/_WKTextManipulationItem/init()``
- ``WKInternalsNotes/_WKTextManipulationItem/initWithIdentifier(_:tokens:)``
- ``WKInternalsNotes/_WKTextManipulationItem/initWithIdentifier(_:tokens:isSubframe:isCrossSiteSubframe:)``
- ``WKInternalsNotes/_WKTextManipulationItem/isEqualToTextManipulationItem(_:includingContentEquality:)``

## Metadata
| Key | Value |
| --- | ----- |
| Status | Draft |
| Last updated | 2025-12-19 |
| WebKit revision | [`WebKit-7623.1.14.10.9`](https://github.com/WebKit/WebKit/tree/WebKit-7623.1.14.10.9) |
| Header (WebKit repo-relative path) | [`_WKTextManipulationItem.h`](https://github.com/WebKit/WebKit/blob/WebKit-7623.1.14.10.9/Source/WebKit/UIProcess/API/Cocoa/_WKTextManipulationItem.h) |
