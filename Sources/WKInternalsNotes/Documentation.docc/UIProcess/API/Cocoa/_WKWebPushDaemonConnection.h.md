# ``WKInternalsNotes/_WKWebPushDaemonConnection``

## Topics

### Type

- ``WKInternalsNotes/_WKWebPushDaemonConnection/cancelNotification(_:uuid:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/getNextPendingPushMessage(_:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/getNotifications(_:tag:completionHandler:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/getPushPermissionStateForOrigin(_:completionHandler:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/getSubscriptionForScope(_:completionHandler:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/init()``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/initWithConfiguration(_:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/new()``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/requestPushPermissionForOrigin(_:completionHandler:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/setAppBadge(_:origin:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/showNotification(_:completionHandler:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/subscribeToPushServiceForScope(_:applicationServerKey:completionHandler:)``
- ``WKInternalsNotes/_WKWebPushDaemonConnection/unsubscribeFromPushServiceForScope(_:completionHandler:)``

## Metadata
| Key | Value |
| --- | ----- |
| Status | Draft |
| Last updated | 2025-12-19 |
| WebKit revision | [`WebKit-7623.1.14.10.9`](https://github.com/WebKit/WebKit/tree/WebKit-7623.1.14.10.9) |
| Header (WebKit repo-relative path) | [`_WKWebPushDaemonConnection.h`](https://github.com/WebKit/WebKit/blob/WebKit-7623.1.14.10.9/Source/WebKit/UIProcess/API/Cocoa/_WKWebPushDaemonConnection.h) |
