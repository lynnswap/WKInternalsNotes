# ``WKInternalsNotes/_WKActivatedElementInfo``

## Topics

### Properties (from _WKActivatedElementInfo.h)

- ``WKInternalsNotes/_WKActivatedElementInfo/boundingRect``
- ``WKInternalsNotes/_WKActivatedElementInfo/canShowAnimationControls``
- ``WKInternalsNotes/_WKActivatedElementInfo/ID``
- ``WKInternalsNotes/_WKActivatedElementInfo/image``
- ``WKInternalsNotes/_WKActivatedElementInfo/imageURL``
- ``WKInternalsNotes/_WKActivatedElementInfo/isAnimatedImage``
- ``WKInternalsNotes/_WKActivatedElementInfo/isAnimating``
- ``WKInternalsNotes/_WKActivatedElementInfo/isSpatialImage``
- ``WKInternalsNotes/_WKActivatedElementInfo/title``
- ``WKInternalsNotes/_WKActivatedElementInfo/type``
- ``WKInternalsNotes/_WKActivatedElementInfo/URL``
- ``WKInternalsNotes/_WKActivatedElementInfo/userInfo``

### Properties (from _WKActivatedElementInfoInternal.h)

- ``WKInternalsNotes/_WKActivatedElementInfo/_animationsUnderElement``
- ``WKInternalsNotes/_WKActivatedElementInfo/_interactionLocation``
- ``WKInternalsNotes/_WKActivatedElementInfo/_isImage``
- ``WKInternalsNotes/_WKActivatedElementInfo/_isUsingAlternateURLForImage``
- ``WKInternalsNotes/_WKActivatedElementInfo/imageMIMEType``

### Methods

- ``WKInternalsNotes/_WKActivatedElementInfo/_initWithInteractionInformationAtPosition(_:isUsingAlternateURLForImage:userInfo:)``
- ``WKInternalsNotes/_WKActivatedElementInfo/_initWithType(_:image:information:)``
- ``WKInternalsNotes/_WKActivatedElementInfo/_initWithType(_:URL:image:information:)``
- ``WKInternalsNotes/_WKActivatedElementInfo/_initWithType(_:URL:imageURL:information:)``
- ``WKInternalsNotes/_WKActivatedElementInfo/_initWithType(_:URL:imageURL:location:title:ID:rect:image:imageMIMEType:isAnimatedImage:isAnimating:canShowAnimationControls:animationsUnderElement:userInfo:)``
- ``WKInternalsNotes/_WKActivatedElementInfo/_initWithType(_:URL:imageURL:userInfo:information:)``
- ``WKInternalsNotes/_WKActivatedElementInfo/_initWithType(_:URL:information:)``
- ``WKInternalsNotes/_WKActivatedElementInfo/activatedElementInfoWithInteractionInformationAtPosition(_:userInfo:)``

## Metadata
| Key | Value |
| --- | ----- |
| Status | Draft |
| Last updated | 2025-12-30 |
| WebKit revision | [`WebKit-7623.1.14.10.9`](https://github.com/WebKit/WebKit/tree/WebKit-7623.1.14.10.9) |
| Header (WebKit repo-relative path) | [`_WKActivatedElementInfo.h`](https://github.com/WebKit/WebKit/blob/WebKit-7623.1.14.10.9/Source/WebKit/UIProcess/API/Cocoa/_WKActivatedElementInfo.h) |
