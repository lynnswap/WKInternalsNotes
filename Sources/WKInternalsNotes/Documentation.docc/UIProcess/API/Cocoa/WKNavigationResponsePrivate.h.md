# ``WKInternalsNotes/WKNavigationResponse``

WKNavigationResponse の Objective-C private/testing API をカテゴリ別に整理した一覧。

## Topics

### WKPrivate

- ``WKInternalsNotes/WKNavigationResponse/_downloadAttribute``
- ``WKInternalsNotes/WKNavigationResponse/_frame``
- ``WKInternalsNotes/WKNavigationResponse/_isFromNetwork``
- ``WKInternalsNotes/WKNavigationResponse/_navigation``
- ``WKInternalsNotes/WKNavigationResponse/_navigationInitiatingFrame``
- ``WKInternalsNotes/WKNavigationResponse/_proxyName``
- ``WKInternalsNotes/WKNavigationResponse/_request``
- ``WKInternalsNotes/WKNavigationResponse/_wasPrivateRelayed``

## Metadata
| Key | Value |
| --- | ----- |
| Status | Draft |
| Last updated | 2025-12-19 |
| WebKit revision | [`WebKit-7623.1.14.10.9`](https://github.com/WebKit/WebKit/tree/WebKit-7623.1.14.10.9) |
| Header (WebKit repo-relative path) | [`WKNavigationResponsePrivate.h`](https://github.com/WebKit/WebKit/blob/WebKit-7623.1.14.10.9/Source/WebKit/UIProcess/API/Cocoa/WKNavigationResponsePrivate.h) |
